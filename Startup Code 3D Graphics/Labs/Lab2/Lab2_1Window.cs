...
namespace Labs.Lab2
{
    ...
    {        
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            #endregion
       /*     #region triangle
            float[] vertices = new float[] { -0.8f, 0.8f, 0.4f, 0.5f, 0.5f, 1.0f,
                                             -0.6f, -0.4f, 0.4f, 1.0f, 0.5f, 1.0f,
                                             0.2f, 0.2f, 0.4f, 1.0f, 0.0f, 0.5f };
       ...
       */
            ...
            float[] SQvertices = new float[] { -0.2f, -0.4f, 0.2f, 0.0f, 1.0f, 0.0f,
                                               0.8f, -0.4f, 0.2f, 0.0f, 0.0f, 1.0f,
                                               0.8f, 0.6f, 0.2f, 1.0f, 1.0f, 1.0f,
                                               -0.2f, 0.6f, 0.2f, 0.0f, 1.0f, 0.0f};

            uint[] SQindices = new uint[] { 0, 1, 2, 3 };
            ...
            GL.BufferData(BufferTarget.ArrayBuffer, (IntPtr)(SQvertices.Length * sizeof(float)), SQvertices, BufferUsageHint.StaticDraw);

            int squareSize;
            GL.GetBufferParameter(BufferTarget.ArrayBuffer, BufferParameterName.BufferSize, out squareSize);

            if (SQvertices.Length * sizeof(float) != squareSize)
            {
                throw new ApplicationException("Vertex data not loaded onto graphics card correctly");
            }
            ...
            if (SQindices.Length * sizeof(int) != squareSize)
            {
                throw new ApplicationException("Index data not loaded onto graphics card correctly");
            }
